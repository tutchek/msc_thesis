...
Based on the description if the previous sections, we now formulate an {\em Automated logistics problem} (ALP\glos{ALP}{Automated logistics problem}), which is to be solved in this thesis. Given a
5-tuple $({\bf G}, {\bf S}, {\bf Del}, {\bf D}, OnStock)$, where:
\begitems
...
$$\hskip\displaywidth minus 1fill\Rightarrow\forall d' <_{\bf D} d \wedge {\rm store}(d) = {\rm store}(d')\colon {\rm amount}(d') =\hskip-2em \sum_{\scriptstyle p' \in {\bf Sch}\atop \scriptstyle 
{\rm demand}(p') = d'}{\hskip-2em {\rm amount}(p)} \eqmark$$

* All possible transfers must be scheduled.

$$\forall g \in {\bf G}\colon \sum_{p \in {\bf Sch}\atop{\rm goods}(p) = g}{{\rm amount}(p)} = \min\left\{\sum_{s \in {\bf S}}{OnStock(g,s)}; \sum_{d\in{\bf D}\atop {\rm goods}(d) = g}{{\rm amount}(d)} \right\}\eqmark $$
...
introduce an {\em Automated logistics problem with low-priority demands} (ALP-lp\glos{ALP-lp}{Automated logistics problem with low-priority demands}) as a variation to ALP.
...
[multikriterialni, zmeni se podminky....]
